...
# from summarizer import summarize_text, summarize_pdf_cached, get_summary_length_estimate
...
#     with col1:
#         input_method = st.radio("Input:", ["📝 Text", "📄 PDF"], horizontal=True)
#     with col2:
#         summary_type = st.selectbox("Style:", ["concise", "detailed", "bullet"])
    
#     text_to_summarize = ""
#     pdf_hash = None
    
#     if input_method == "📝 Text":
#         text_to_summarize = st.text_area("Enter text:", height=300)
...
#                 text_to_summarize = read_pdf_util(uploaded_file)
...
#             st.subheader("📋 Summary:")
#             st.markdown(f"<div style='background:#f0f2f6;padding:1.5rem;border-radius:0.5rem;'>{summary}</div>", unsafe_allow_html=True)
...
#             )
#         else:
#             st.warning("⚠️ Provide at least 50 characters")
...
#             st.markdown(f"<div class='chat-message assistant-message'><strong>AI:</strong><br>{a}</div>", unsafe_allow_html=True)
...
#                     answer, sources = chat_with_pdf(uploaded_pdf, user_question, current_hash)
